...
#dilation = cv2.dilate(warped2, kernel1, 1)
opening = cv2.morphologyEx(warped2, cv2.MORPH_OPEN, kernel1)
closing = cv2.morphologyEx(warped2, cv2.MORPH_CLOSE, kernel1)
#dilation = cv2.dilate(dilation, kernel2, 1)
#binary_sauvola = binary_sauvola.astype("uint8")*255
save_file = '{}{}{}'.format(path, '_transformed', ext)

#show the original and scanned images
print("STEP 3 :Apply perspective transform")
#cv2.imshow("Original", imutils.resize(orig, height = 650))
cv2.imshow("OriginalWarped", copy_of_orig)
...
cv2.imshow("Opening Warp", imutils.resize(opening, height = 650))
cv2.imshow("Closing Warp", imutils.resize(closing, height = 650))
